...
function calcDist(lat1, lng1, lat2, lng2) {
  ...
  const R = 6370.139; // (km) at lat = -37.81895485084791
  const dLat = degsToRads(lat2 - lat1);
  const dLng = degsToRads(lng2 - lng1);
  const lat1Rad = degsToRads(lat1);
  const lat2Rad = degsToRads(lat2);
...
function getMaxTier(site) {
  ...
  const tier = parseInt(exposureWithMaxTier.tier);
  if (isNaN(tier)) {
    return '?';
  } else {
    return tier;
...
function clearTable() {
  els.sitesTable.getElementsByTagName('tbody')[0].innerHTML = '';
}
...
function populateTable(sites, userPos) {
  ...
  const table = document.getElementById('sites').getElementsByTagName('tbody')[0];
...
function hideAllPositionToasts() {
  ...
  toasts.posPermissionDenied.hide();
  toasts.posUnavailable.hide();
  toasts.posTimeout.hide();
...
async function getUserPosition() {
  ...
  if (!userPosLastUpdated || parseInt(userPosLastUpdated) < +timeNow - minsToMs(maxAgeMins)) {
    const options = {
      ...
      timeout: 10000,
      maximumAge: minsToMs(maxAgeMins),
    };
    return new Promise((resolve, reject) => {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          if (error.code === 1) {
            toasts.posPermissionDenied.show();
          } else if (error.code === 2) {
            toasts.posUnavailable.show();
          } else if (error.code === 3) {
            toasts.posTimeout.show();
          }
        },
        options
      );
...
async function fetchSites(state) {
  return sitesEndpoint({ state: state }).then((response) => {
...
async function getSites(state) {
  ...
  if (state === 'vic') {
    // That day has come! No more VIC sites: https://www.coronavirus.vic.gov.au/case-alerts-public-exposure-sites
    return [];
  }

  let downloadingSitesToast;
  if (state === 'vic') {
    downloadingSitesToast = toasts.vicDownloadingSites;
  } else if (state == 'nsw') {
    downloadingSitesToast = toasts.nswDownloadingSites;
  }
  downloadingSitesToast.show();

  const sitesVal = getCachedSites(state);
  if (sitesVal && sitesVal.results.length > 0) {
    ...
    return fetchSites(state).then((sitesVal) => {
...
els.useLocationBtn.addEventListener('click', () => {
  activateLocBtn(els.useLocationBtn);
  deactivateLocBtn(els.useAddressBtn);
  deactivateLocBtn(els.useRecentAddress); // deactivate the div
...
els.useAddressBtn.addEventListener('click', () => {
  activateLocBtn(els.useAddressBtn);
  deactivateLocBtn(els.useLocationBtn);
  deactivateLocBtn(els.useRecentAddress); // deactivate the div
  deactivateAllRecentAddressBtns(); // deactivate any active recent address btns too
...
function useRecentAddressClicked() {
  activateLocBtn(els.useRecentAddress);
  deactivateLocBtn(els.useLocationBtn);
  deactivateLocBtn(els.useAddressBtn);
...
els.nswCheckbox.addEventListener('change', () => {
  if (els.nswCheckbox.checked) {
    showState('nsw');
  } else {
    dontShowState('nsw');
    if (!els.vicCheckbox.checked) {
      clearShowingSitesMsg();
    }
  }
});

els.vicCheckbox.addEventListener('change', () => {
  if (els.vicCheckbox.checked) {
    showState('vic');
  } else {
    dontShowState('vic');
...
els.saveUserLocModal.addEventListener('hidden.bs.modal', function (event) {
  console.log('modal closed!');
});

function showState(state) {
  window.localStorage.setItem(`show${state}`, true);
  const sites = getSites(state).then((sitesVal) => {
...
function addAddressToRecentWidget(address) {
  ...
  const clonedElement = clone.querySelector('div'); // get it's first element which is a div

  clonedElement.id = hash;
  const addressBtn = clonedElement.firstElementChild;
  const closeBtn = clonedElement.lastElementChild;
...
function removeNoRecentAddressesMsg() {
  const msgElement = els.noRecentAddressMsg();
  msgElement && msgElement.remove();
}

function cacheAddress(address, lat, lng) {
  const addresses = window.localStorage.getItem('recentAddresses');
...
function setShowingSitesMsg(msg) {
  const showingSitesElement = document.getElementById('showingSites');
  showingSitesElement.innerHTML = msg;
}

function clearShowingSitesMsg() {
  const showingSitesElement = document.getElementById('showingSites');
  showingSitesElement.innerHTML = '';
}
...
async function updateTable() {
  ...
  if (totalSites && totalSites.length > 0) {
    ...
    let numExposures = filteredSites.reduce((a, b) => {
      return a + b.exposures.length;
    }, 0);
    ...
    if (filteredSites.length < 20) {
      ...
      numExposures = filteredSites.reduce((a, b) => {
        return a + b.exposures.length;
      }, 0);
      setShowingSitesMsg(
        `Showing the closest <span class="fs-5">${filteredSites.length}</span> sites with <span class="fs-5">${numExposures}</span> exposures`
      );
    } else {
      setShowingSitesMsg(
        `Showing <span class="fs-5">${filteredSites.length}</span> sites with <span class="fs-5">${numExposures}</span> exposures within 10km`
      );
    }
...
function addStateInfo(state, numSites, numExposures, lastUpdated) {
  ...
  if (state === 'nsw') {
    ...
    lastUpdatedMsg = `<p class="m-1" id="${state}LastUpdated"><small>Updated ${prettyTime(
      lastUpdated
    )} using <a href="https://www.health.nsw.gov.au/Infectious/covid-19/Pages/case-locations-and-alerts.aspx" target="_blank">NSW Health</a> data.</small></p>`;
  } else if (state === 'vic') {
    stateFullName = 'Victoria';
    lastUpdatedMsg = `<p class="m-1" id="${state}LastUpdated"><small>Updated ${prettyTime(
      lastUpdated
    )} using <a href="https://www.coronavirus.vic.gov.au/exposure-sites" target="_blank">Victorian Department of Health</a> data.</small></p>`;
  }
